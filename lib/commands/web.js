...
extensions.getExtraNativeWebTapShadowOffset = async function getExtraNativeWebTapShadowOffset (el) {
  let offset = 0;

  try {
    // go through the hierarchy for the element, to see if it is within a
    // shadow root
    const isShadow = await this.execute(`return (function isInShadow(node) {
  ...
  return false;
})(arguments[0])`, [{ELEMENT: el}]);
    if (isShadow) {
      offset -= SHADOW_DOM_Y_OFFSET;
      log.debug(`Element is in Shadow DOM. Additional native web tap offset computed: ${offset}`);
    }
  } catch (ign) {}
...
extensions.nativeWebTap = async function nativeWebTap (el) {
  ...
  // this has to be done here, since we need to have the element, and
  // everything else is computing based on the state of the app/device
  const yOffset = await this.getExtraNativeWebTapShadowOffset(el);
  y += yOffset;
